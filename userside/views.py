...
def womenproduct(request,wp):
    if request.method == "POST":
        ...
        return render(request,"userside/single.html",{'product':pro,'wp':wp})
...
def add_to_cart(request,mk):
    ...
    return render(request,"userside/single.html",{'products':pro,'mk':cats})
...
def showcart(request):
    try:
        ...
        print(newcart)
        return render(request,"userside/showcart.html",{"cart":newcart})
    except:
...
def checkout(request):
    if request.method=="POST":
        ...
        return render(request,"userside/checkout.html",{"user":user})

@login_required(login_url="/")
def history(request):
    orders = Order.objects.filter(userid=request.user)
    return render(request,"userside/history.html",{"orders":orders})
...
def viewdetails(request,id):
    ...
    for i,j in (json.loads(order.products)).items():
        ...
        total += int(j)*int(Product.objects.get(id=i).original_price)
    print(order_details)
    return render(request,"userside/vieworder.html",{"order":order,"order_details":order_details,"total":total})